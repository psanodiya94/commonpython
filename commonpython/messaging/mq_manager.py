...
class MQManager:
    ...
    def _execute_mq_command(self, command: str, params: Optional[List[str]] = None) -> Dict[str, Any]:
        """
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if result['success']:
                self.connected = True
                if self.logger:
                    self.logger.logger.info(f"Successfully connected to MQ queue manager: {self.connection_info['queue_manager']}")
                return True
            else:
                if self.logger:
                    self.logger.logger.error(f"Failed to connect to MQ queue manager: {result['stderr']}")
                return False
    ...
    def put_message(self, queue_name: str, message: Union[str, bytes, Dict], 
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if self.logger:
                    self.logger.logger.error(f"Error putting message to queue {queue_name}: {result.stderr}")
                return False
    ...
    def browse_message(self, queue_name: str, message_id: Optional[bytes] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            duration = time.time() - start_time
            if self.logger:
                self.logger.logger.error(f"Error browsing message from queue {queue_name}: {str(e)}")
                self.logger.log_mq_operation(
                    operation="BROWSE",
                    queue=queue_name,
                    duration=duration
    ...
    def get_queue_depth(self, queue_name: str) -> int:
        ...
        try:
            ...
            if result['success']:
                ...
                if self.logger:
                    self.logger.logger.error(f"Error getting queue depth for {queue_name}: {result['stderr']}")
                return -1
    ...
    def purge_queue(self, queue_name: str) -> int:
        ...
        try:
            ...
            if result['success']:
                ...
                if self.logger:
                    self.logger.logger.error(f"Error purging queue {queue_name}: {result['stderr']}")
