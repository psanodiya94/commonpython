...
class TestCLI(unittest.TestCase):
    ...
    def test_setup_database_success(
        self, mock_logger_manager_class, mock_config_manager_class, mock_db_manager_class
    ):
        """
    ...
    @patch("commonpython.config.config_manager.ConfigManager")
    @patch("commonpython.logging.logger_manager.LoggerManager")
    def test_setup_database_failure(
        self, mock_logger_manager_class, mock_config_manager_class, mock_db_manager_class
    ):
        """
    ...
    @patch("commonpython.config.config_manager.ConfigManager")
    @patch("commonpython.logging.logger_manager.LoggerManager")
    def test_setup_messaging_success(
        self, mock_logger_manager_class, mock_config_manager_class, mock_mq_manager_class
    ):
        """
    ...
    @patch("commonpython.config.config_manager.ConfigManager")
    @patch("commonpython.logging.logger_manager.LoggerManager")
    def test_setup_messaging_failure(
        self, mock_logger_manager_class, mock_config_manager_class, mock_mq_manager_class
    ):
        """
    ...
    def test_test_database_success(self, mock_setup):
        ...
        with patch("builtins.print") as mock_print:
            ...
            calls = [
                call("Testing database connection..."),
                call("✓ Database connection successful"),
            ]
    ...
    def test_execute_query_update(self, mock_setup):
        ...
        with patch("builtins.print") as mock_print:
            ...
            self.cli.db_manager.execute_update.assert_called_once_with(
                "UPDATE test SET col = 1", None
            )
    ...
    def test_execute_query_with_multiple_params(self, mock_setup):
        ...
        with patch.object(self.cli, "_display_results"):
            ...
            self.cli.db_manager.execute_query.assert_called_once_with(
                "SELECT * FROM test WHERE id = ? AND name = ?", ("1", "test")
            )
...
class TestCreateParser(unittest.TestCase):
    ...
    def test_parser_database_execute_with_params(self):
        ...
        args = parser.parse_args(
            ["database", "execute", "SELECT * FROM test WHERE id = ?", "--params", '["1"]']
        )
    ...
    def test_parser_messaging_put_with_properties(self):
        ...
        parser = create_parser()
        args = parser.parse_args(
            ["messaging", "put", "TEST.QUEUE", "msg", "--properties", '{"priority": 5}']
        )
